...
import {getSkillsDetails, getSkillsDetailsSuccess, getSkillsPickList} from "@/redux/skills/action-reducer.js";
...
export default function SkillFilter() {
    ...
    const {pickList, skillsDetails} = useSelector(state => state.skills);
    ...
    const sortSkills = (option) => {
        let skills = [...skillsDetails];
        switch (option) {
            case 'hl':
                skills.sort((a, b) => b.skillProficiency - a.skillProficiency);
                break;
            case 'lh':
                skills.sort((a, b) => a.skillProficiency - b.skillProficiency);
                break;
            case 'zta':
        ...
        dispatch(getSkillsDetailsSuccess(skills))
    }

    return (
        <>
            <div className="relative sm:block">
                <SearchInput placeholder={"Search Skills"} onSearch={(val) => setSearch(val)}/>
            </div>
                        ...
                        <RadioGroup defaultValue="comfortable" className="space-y-2">
                            {skillsList.map((skill, index) => (
                                <div className="flex items-center space-x-2 " key={index}>
                                    <Checkbox id={`s${index + 1}`} onCheckedChange={(v) => setSkillsList(prevState => {
                                        let old = [...prevState];
                                    ...
                                    />
                                    <Label className={'cursor-pointer'} htmlFor={`s${index + 1}`}>{skill.name}</Label>
                                </div>
                            ))}
                        </RadioGroup>
            ...
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <Button variant="outline"
                        ...
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                             xmlns="http://www.w3.org/2000/svg">
                            <path d="M4.34766 8H11.6527" stroke="#292929"
                                  strokeWidth="1.5" strokeLinecap="round"
                                  strokeLinejoin="round"/>
                            <path d="M1.83789 4.39941L14.1631 4.39942" stroke="#292929"
                                  strokeWidth="1.5"
                                  strokeLinecap="round"
                                  strokeLinejoin="round"/>
                            <path d="M6.91309 11.6006H9.08685" stroke="#292929"
                                  strokeWidth="1.5" strokeLinecap="round"
                                  strokeLinejoin="round"/>
                        </svg>
                    </Button>
