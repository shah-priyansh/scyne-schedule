...
import {getPendingEngagements, getPendingEngagementsFiltered} from "@/redux/endorsements/action-reducer.js";
...
export default function EndorsementPendingFilter() {
    ...
    useEffect(() => {
        setSkillsList(pendingEngagements.map(f => ({isChecked: false, name: f.name, id: f.id})));
    }, [pendingEngagements]);

    const filterEndorsements = useCallback(() => {
        dispatch(getPendingEngagements({
            search: search,
        }));
    }, [search, dispatch]);

    const filterEndorsementsPersons = () => {
        let old = [...pendingEngagements]
        let filter = skillsList.filter(d => d.isChecked).map(f => f.id);
        if (filter.length > 0) {
            old = old.filter(j => filter.includes(j.id));
        }
        dispatch(getPendingEngagementsFiltered(old));
    }
    ...
    const sortSkills = (option) => {
        let endorsements = [...pendingEngagements];
        switch (option) {
            case 'hl':
                endorsements.sort((a, b) => b.skills.length - a.skills.length);
                break;
            case 'lh':
                endorsements.sort((a, b) => a.skills.length - b.skills.length);
                break;
            case 'zta':
        ...
        dispatch(getPendingEngagements(endorsements))
    }
