import conn from './connnection.js';

// ==============================
// CONSTANTS
// ==============================
export const TOTAL_SKILLS_RECORDS = 'TOTAL_SKILLS_RECORDS';
export const TOTAL_MASTERY_LEVEL = 'TOTAL_MASTERY_LEVEL';
export const AVERAGE_SKILL_SCORE = 'AVERAGE_SKILL_SCORE';
export const GET_ALL_SKILLS_INFO = 'GET_ALL_SKILLS_INFO';
export const CERTIFICATE_IMAGES = 'CERTIFICATE_IMAGES';
export const PROJECT_IMAGES = 'PROJECT_IMAGES';
export const FULL_SKILL_DETAILS = 'FULL_SKILL_DETAILS';

export const ADVANCED = 'Advanced';
export const INTERMEDIATE = 'Intermediate';

// ==============================
// IMAGE FETCH FUNCTIONS
// ==============================

export async function fetchRecordImageByLinkedEntity(recordId) {
  const soql = `
    SELECT ContentDocument.LatestPublishedVersionId
    FROM ContentDocumentLink
    WHERE LinkedEntityId = '${recordId}' ORDER BY CreatedDate DESC
    LIMIT 1
  `;
  const res = await conn.query(soql);

  if (res.records.length > 0) {
    const versionId = res.records[0].ContentDocument.LatestPublishedVersionId;
    return `/sfc/servlet.shepherd/version/renditionDownload?rendition=THUMB720BY480&versionId=${versionId}`;
  }
  return null;
}

export async function fetchAllRecordImages(recordId) {
  const soql = `
    SELECT ContentDocument.LatestPublishedVersionId
    FROM ContentDocumentLink
    WHERE LinkedEntityId = '${recordId}' ORDER BY CreatedDate DESC
  `;
  const res = await conn.query(soql);

  if (res.records.length > 0) {
    return res.records.map(
      rec => `/sfc/servlet.shepherd/version/renditionDownload?rendition=THUMB720BY480&versionId=${rec.ContentDocument.LatestPublishedVersionId}`
    );
  }
  return [];
}

// ==============================
// CERTIFICATE FUNCTIONS
// ==============================

export async function getCertificatesBySkill(skillId) {
  const soql = `
    SELECT Id, Name, CertificateIssueDate__c
    FROM Certificate__c
    WHERE SkillId__c = '${skillId}' ORDER BY CreatedDate DESC
  `;
  const res = await conn.query(soql);

  const certificates = await Promise.all(
    res.records.map(async cert => {
      const imageUrl = await fetchRecordImageByLinkedEntity(cert.Id);
      return {
        id: cert.Id,
        name: cert.Name,
        issueDate: cert.CertificateIssueDate__c != null ? cert.CertificateIssueDate__c : '',
        imageUrl
      };
    })
  );

  console.log('certificates :>> ', certificates);
  return {
    type: CERTIFICATE_IMAGES,
    payload: certificates
  };
}

// ==============================
// PROJECT FUNCTIONS
// ==============================

export async function getProjectsBySkill(skillId) {
  const soql = `
    SELECT Id, Name, ProjectDescription__c
    FROM Project__c
    WHERE SkillsId__c = '${skillId}' ORDER BY CreatedDate DESC
  `;
  const res = await conn.query(soql);

  const projects = await Promise.all(
    res.records.map(async proj => {
      const imageUrls = await fetchAllRecordImages(proj.Id);
      return {
        id: proj.Id,
        name: proj.Name,
        description: proj.ProjectDescription__c != null ? proj.ProjectDescription__c : '',
        images: imageUrls
      };
    })
  );

  console.log('projects :>> ', projects);
  return {
    type: PROJECT_IMAGES,
    payload: projects
  };
}

// ==============================
// WRAPPER FUNCTION
// ==============================

export async function getFullSkillDetails(skillId) {
  try {
    const [certificates, projects] = await Promise.all([
      getCertificatesBySkill(skillId),
      getProjectsBySkill(skillId)
    ]);

    const payload = {
      skillId,
      certificates: certificates.payload,
      projects: projects.payload
    };

    console.log("FULL SKILL DETAILS", payload);
    return {
      type: FULL_SKILL_DETAILS,
      payload
    };
  } catch (err) {
    console.error("Error in getFullSkillDetails:", err);
    throw err;
  }
}

// ==============================
// SAMPLE CALLS (for testing)
// ==============================

// getCertificatesBySkill('aA2Kj000008XbiOKAS');
// getProjectsBySkill('aA2Kj000008XbiOKAS');
getFullSkillDetails('aA2Kj000008XbiOKAS');

// --------------------------------------------------
// SAMPLE DATA
// --------------------------------------------------

// Skill record
const skillData = {
  Name: 'React JS Development',
  year: 3,
  masterylevel: 'Intermediate',
  selfSkillRating: 'Good',
  skillProficiency: 75,
  prentSkill: 'Frontend',
  skillDescription: 'Hands-on experience in building scalable React apps.',
  endorsement: 20,
  accountId: '001Kj00002ePEZ5IAO'
};

// Certificates with one file each
const certificateData = [
  {
    certificateName: 'Mastery in React Development',
    certificateLink: 'https://certificates.com/react-dev',
    certificateDescription: 'Completed advanced React course with projects.',
    certificateIssueDate: '2022-06-12',
    accountId: '001Kj00002ePEZ5IAO',
    certificateImage: {
      fileName: 'react_cert.pdf',
      base64Data: ''
    }
  },
  {
    certificateName: 'JavaScript Specialist',
    certificateLink: 'https://certificates.com/js-specialist',
    certificateDescription: 'Deep knowledge of JavaScript ES6+ features.',
    certificateIssueDate: '2021-08-20',
    accountId: '001Kj00002ePEZ5IAO',
    certificateImage: {
      fileName: 'js_cert.png',
      base64Data: ''
    }
  }
];

// Projects with one file each
const projectData = [
  {
    projectTitle: 'InsightEye – Private Investigator App',
    projectDescription: 'Built a React + Salesforce integrated investigation app.',
    timeline: '2021-2022',
    accountId: '001Kj00002ePEZ5IAO',
    ProjectImage: {
      fileName: 'insighteye.jpg',
      base64Data: ''
    }
  },
  {
    projectTitle: 'TaskFlow Automation',
    projectDescription: 'Automated workflows with Salesforce + LWC.',
    timeline: '2020-2021',
    accountId: '001Kj00002ePEZ5IAO',
    ProjectImage: {
      fileName: 'taskflow.png',
      base64Data: ''
    }
  }
];



// --------------------------------------------------
// FUNCTIONS
// --------------------------------------------------

/**
 * Insert a file (ContentVersion) and link it
 */

export async function insertFileAndLink(fileMeta, parentId, title) {
  const { fileName, base64Data } = fileMeta;

  // Create ContentVersion dynamically with provided filename + base64
  const cv = await conn.sobject('ContentVersion').create({
    Title: title,
    PathOnClient: fileName,
    VersionData: base64Data
  });

  if (!cv.success) throw new Error('Failed to insert ContentVersion');

  // Query back ContentDocumentId
  const contentVer = await conn.sobject('ContentVersion').retrieve(cv.id);
  const cdId = contentVer.ContentDocumentId;

  // Link with parent record
  await conn.sobject('ContentDocumentLink').create({
    ContentDocumentId: cdId,
    LinkedEntityId: parentId,
    ShareType: 'V',
    Visibility: "AllUsers",
  });

  console.log(`📎 File '${fileName}' linked to ${parentId}`);
}

/**
 * Create Skill
 */
export async function createSkill(skills) {
  const skillData = {
    Name: skills.Name,
    Year__c: skills.year,
    Masterylevel__c: skills.masterylevel,
    SelfSkillRating__c: skills.selfSkillRating,
    SkillProficiency__c: skills.skillProficiency,
    ParentSkills__c: skills.prentSkill,
    SkillDescription__c: skills.skillDescription,
    Endorsement__c: skills.endorsement,
    AccountId__c: skills.accountId // Lookup to Account
  };
  const ret = await conn.sobject('Skills__c').create(skillData);
  if (ret.success) {
    console.log(`Skill created: ${ret.id}`);
    return ret.id;
  }
  throw new Error('Skill creation failed');
}

/**
 * Create Certificates linked to Skill
 */
export async function createCertificates(skillId, certificates) {
  const certsToInsert = certificates.map(c => ({
    CertificateName__c: c.certificateName,
    Name:c.certificateName,
    CertificateLink__c: c.certificateLink,
    CertificateDescription__c: c.certificateDescription,
    CertificateIssueDate__c: c.certificateIssueDate,
    SkillId__c: skillId, // Lookup to Skill
    AccountId__c: c.accountId // Lookup to Account
  }));

  const rets = await conn.sobject('Certificate__c').create(certsToInsert);

  for (let i = 0; i < rets.length; i++) {
    if (rets[i].success) {
      console.log(`Certificate created: ${rets[i].id}`);
      if (certificates[i].CertificateImage) {
        await insertFileAndLink(
          certificates[i].CertificateImage,
          rets[i].id,
          //certificates[i].CertificateTitle__c
          certificates[i].CertificateName__c	

        );
      }
    }
  }
}

/**
 * Create Projects linked to Skill
 */
export async function createProjects(skillId, projects) {
  const projectsToInsert = projects.map(p => ({
    ProjectTitle__c: p.projectTitle,
    Name: p.projectTitle,
    ProjectDescription__c: p.projectDescription,
    Timeline__c: p.timeline,
    SkillsId__c: skillId, // Lookup to Skill
    AccountId__c: p.accountId // Lookup to Account
  }));

  const rets = await conn.sobject('Project__c').create(projectsToInsert);
  for (let i = 0; i < rets.length; i++) {
    console.log('rets :>> ', rets[i].errors);
    if (rets[i].success) {
      console.log(`Project created: ${rets[i].id}`);
      if (projects[i].ProjectImage) {
        await insertFileAndLink(
          projects[i].ProjectImage,
          rets[i].id,
          projects[i].ProjectTitle__c
        );
      }
    }
  }
}

/**
 * Wrapper function to orchestrate everything
 */
export async function submitSkillWrapper() {
  try {
    // 1. Create skill
    const skillId = await createSkill(skillData);

    // 2. Create certificates
    await createCertificates(skillId, certificateData);

    // 3. Create projects
    await createProjects(skillId, projectData);

    console.log('All records created and linked successfully');
  } catch (err) {
    console.error('Failed:', err);
  }
}

// Run wrapper
await submitSkillWrapper();
