...
/**
 * Update Account, Contact, and Skills data from unified payload
 * @param {object} payload - Combined update payload
 */
export async function updateUserAboutInfo(payload) {
  const {
    accountId,
    accountDetails,
    contactDetails,
  ...
  try {
    ...
    if (Array.isArray(newGroupedSkills)) {
      ...
      if (toDelete.length > 0) {
        await conn.sobject("Skills__c").del(toDelete);
      }
      if (toUpdate.length > 0) {
        await conn.sobject("Skills__c").update(toUpdate);
      }
      if (newSkillsToCreate.length > 0) {
        await conn.sobject("Skills__c").create(newSkillsToCreate);
      }
    }
    ...
    if (profilePicture?.fileName && profilePicture?.base64Data && accountId) {
      ...
      const contentVersionRes = await conn.sobject("ContentVersion").create({
      ...
      const queryRes = await conn.query(`
        ...
        FROM ContentVersion 
        WHERE Id = '${contentVersionId}' ORDER BY CreatedDate DESC
        LIMIT 1
      `);
    ...
    return { success: true, message: "User about info updated successfully." };
  } catch (err) {
